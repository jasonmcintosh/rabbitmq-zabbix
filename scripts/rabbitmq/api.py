...
class RabbitMQAPI(object):
    ...
    def __init__(self, user_name='guest', password='guest', host_name='', aliveness_vhost='',
                 port=15672, conf='/etc/zabbix/zabbix_agentd.conf', senderhostname=None, protocol='http'):
        self.user_name = user_name
        self.password = password
        self.host_name = host_name or socket.gethostname()
        self.port = port
	self.aliveness_vhost = aliveness_vhost or '%2f'
    ...
    def check_aliveness(self):
        ...
        return self.call_api('aliveness-test/'+self.aliveness_vhost)['status']
...
def main():
    ...
    parser.add_option('--aliveness_vhost', help='RabbitMQ vhost for check aliveness', default='%2f')
    ...
    api = RabbitMQAPI(user_name=options.username, password=options.password,
                      host_name=options.hostname, port=options.port, aliveness_vhost=options.aliveness_vhost,
